...
function Anima:init()
    local obj={stage=nil,mark={x=0,y=0,r=0,sx=0,sy=0,op=0},key={x=0,y=0,r=0,sx=0,sy=0,op=0},x=0,y=0,r=0,sx=0,sy=0,op=0,animStart=false,animOver=false,animEnd=false}
    ...
    for i in ipairs(animtbl) do
        ...
        if not self.stage or (self:animationOver() and self.stage==i) then
            ...
            if cmts and funcs then
                ...
                assert(type(cmts)=='table',"Oops! Table expected in #3rd argument, got '"..type(cmts).."''")
